  ...
  export default {
    ...
    methods: {
      ...
      submissionWork() {
        var _this = this;
        //添加
        this.$axios.post("/worksubmission/add", this.workSubmission
        ).then(resp =>
        {
      ...
      loadWorks(){
        //获取已发布的作业
        var _this = this;
        //之前一直报错的原因是因为自己直接复制代码，却没注意_this一直没定义。
        //获取未提交的作业
        this.$axios
          .get('/workrelease', {params:{
              ...
              studentNo:this.workSubmission.studentNo,
              //teacherNo:20200001,
            },dataType:"text"})
          .then(resp => {
        ...
        //alert(data.records);
       // _this.totalCount = 13;
       _this.works = eval(data.records);
        _this.totalCount = data.size;
        //用于接收后端是否正确将数据传到前端。
        console.info(_this.works);
      }).catch(failResponse => {
          //alert(555);
        })
      }
