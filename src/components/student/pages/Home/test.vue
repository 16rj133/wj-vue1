<template>
  <div class="home">
    ...
    <div class="calendar" @click="toCalendar">
      ...
      <div class="calendar-right" v-if="examCalendar[0]">
        <i class="iconfont iconxiazai41"></i>
        最新考试信息：{{examCalendar[0].teaName}}：{{examCalendar[0].noticeContent}}
      </div>
    </div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    computed:{
      //轮播图的数组
      ...mapState(['examCalendar','rotationImages','user','languagesInfo'])
    },
    methods:{
      ...
      toCalendar(){
        console.log(this.$store.state.examCalendar);
        if (!this.$store.state.user.id){
          ...
          this.$router.push('/exam/examcalendar')
        }
      },
      toPaper(langId){
        if (!this.$store.state.user.id){
